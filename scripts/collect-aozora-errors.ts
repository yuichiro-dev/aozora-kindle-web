...
// inline.ts を直接読み込む（またはトランスパイル済み/プロジェクトの形式に合わせる）
const { renderInline } = require('../lib/epub/aozora/inline');
...
async function runErrorCollection() {
  ...
  console.warn = (...args) => {
    ...
    if (msg.includes('[Aozora]')) {
      ...
      // ⚠️ 警告の発生源となった実際のテキスト行をコンソールに直接出力
  ...
  try {
    ...
    console.log(`✅ インデックス解析完了: 全 ${allBooks.length} 件から ${SAMPLE_COUNT} 件をランダム抽出し検証します。\n`);
    ...
    for (let i = 0; i < targetBooks.length; i++) {
      ...
      for (let l = 0; l < lines.length; l++) {
        ...
        try {
            ...
            if (annotations) {
              for (const ann of annotations) {
                ...
                if (!exists) {
                  ...
                  // ⚠️ notesクラスとして拾われた未対応注記の生テキスト行をコンソールに出力
          ...
          // ❌ クラッシュ（実行エラー）発生時の生テキスト行をコンソールに出力
    ...
    if (topUnknownAnnotations.length > 0) {
      ...
      topUnknownAnnotations.slice(0, 10).forEach((item, idx) => {
        console.log(`  ${idx + 1}. ${item.annotation.padEnd(25)} (${item.count} 回) -> 例: ${item.sampleTitle}`);
